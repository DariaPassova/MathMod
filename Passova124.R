...
tbl 
#Посмотрим на сами переменные и для этого воспользуеся функцией glimpse(), которая более наглядно представляет каждую отдельную переменную
glimpse(tbl) 
...
#Сделаем такое последовательно много раз(т.к. нам нужно избавиться не только от !), что приведет к совершенно не читаемому результату. Поэтому воспользуеvся оператором пайппинга
names(tbl) = names(tbl) %>% 
...
#Получм таблицу содержащую все остальные колонки
